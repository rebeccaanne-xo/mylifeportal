 ...
 * test.js
 * ─────────────────────────────────────
 * Job:      Development/testing copy of the portal (not for production) — all interactivity and API calls
 * Connects: Loaded by test/index.html · calls rebecca-portal-api Worker via fetch
...
function loadScripture(){
  ...
  document.getElementById('scripture-verse').textContent='\u201c'+s.t+'\u201d';
  document.getElementById('scripture-cite').textContent=s.r;
}
...
function loadDate(){
  ...
  var greetings={
    morning:['Good morning, Rebecca.','Rise and shine, Rebecca.','Good to see you, Rebecca.','A new day, Rebecca.','Morning, Rebecca. Let\'s go.','Today is yours, Rebecca.'],
    afternoon:['Good afternoon, Rebecca.','Keep going, Rebecca.','Good to see you, Rebecca.','Hey there, Rebecca.','You\'re doing great, Rebecca.','Afternoon, Rebecca.'],
    evening:['Good evening, Rebecca.','Wind down well, Rebecca.','Good to see you, Rebecca.','Evening, Rebecca.','Rest is coming, Rebecca.','Almost there, Rebecca.'],
    night:['Still at it, Rebecca?','Good night, Rebecca.','Rest well, Rebecca.','Proud of you, Rebecca.','Good to see you, Rebecca.']
  };

  var pool=h<12?greetings.morning:h<17?greetings.afternoon:h<21?greetings.evening:greetings.night;
  var greeting=pool[d.getDay()%pool.length];
...
function renderPriorityCards(){
  ...
  allTaskKeys.forEach(function(key){
    ...
    tasks.filter(function(t){
      return (t.priority_flag || t.priority_flag === '1' || t.priority_flag === '2' || t.priority_flag === '3') 
        && t.priority_flag !== '' && !t.done && !t.archived;
    }).forEach(function(t){
      var tid = String(t.id);
      if(seen[tid]) return;
      seen[tid] = true;
...
var SECTION_TO_PROJECT = {
  ...
  ms_tasks:'mustard', hl_tasks:'home', personal:'personal',
...
async function loadAllTasksFromD1() {
  if(!DB_ONLINE) return;
  var sections = ['biz_tasks','wtc_tasks','al_tasks','ms_tasks','hl_tasks','personal','followup_list'];
...
function renderProjectTasks(){
  var sections = [
    {listId:'biz-tasks-list', key:'biz_tasks', project:'gbb'},
    {listId:'al-list', key:'al_tasks', project:'aligned'},
    {listId:'wtc-list', key:'wtc_tasks', project:'wtc'},
    {listId:'ms-list', key:'ms_tasks', project:'mustard'},
    {listId:'hl-task-list', key:'hl_tasks', project:'home'},
    {listId:'fu-list', key:'followup_list', project:'personal'},
  ];
  sections.forEach(function(s){
    ...
    var tasks = ld(s.key, []);
    if(!tasks.length){
      list.innerHTML = '<div style="font-size:11px;color:rgba(255,255,255,.2);font-style:italic;padding:.3rem 0">No tasks yet — add in Project Board</div>';
      return;
    }
    // Show undone tasks first, then done
    var sorted = tasks.slice().sort(function(a,b){
      if(a.done && !b.done) return 1;
      if(!a.done && b.done) return -1;
      return 0;
    });
    sorted.slice(0,10).forEach(function(task){
      list.appendChild(makeCheck(task, s.key));
    });
    if(sorted.length > 10){
      var more = document.createElement('div');
      more.style.cssText = 'font-size:11px;color:rgba(255,255,255,.25);padding:.3rem 0;font-style:italic';
      more.textContent = '+ ' + (sorted.length-10) + ' more in Project Board';
